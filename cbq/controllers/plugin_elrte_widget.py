...
class ElrteWidget(object):
    ...
    def __call__(self, field, value, **attributes):
        if not self.settings.files:
            _files = [
                URL('cbq', 'static', 'plugin_elrte_widget/css/elrte.min.css'),
                URL('cbq', 'static', 'plugin_elrte_widget/css/elrte-inner.css'),
                URL('cbq', 'static', 'plugin_elrte_widget/css/smoothness/jquery-ui-1.8.13.custom.css'),
                URL('cbq', 'static', 'plugin_elrte_widget/js/jquery-ui-1.8.16.custom.min.js'),
                URL('cbq', 'static', 'plugin_elrte_widget/js/elrte.min.js'),
            ]
            if self.settings.lang:
                _files.append(URL('cbq', 'static', 'plugin_elrte_widget/js/i18n/elrte.%s.js' % self.settings.lang))
        else:
            _files = self.settings.files
        _set_files(_files)

        from gluon.utils import web2py_uuid
        _id = '%s_%s_%s' % (field.tablename, field.name, web2py_uuid())
...
""" % dict(id=_id, lang=self.settings.lang or '',
        ...
        return SPAN(script, TEXTAREA((value is not None and str(value)) or '', **attr), **attributes)
