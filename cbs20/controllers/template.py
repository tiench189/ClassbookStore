...
ROOT_DIC = "/home/www-data/"
HOME_DIC = ROOT_DIC + "web2py/applications/cbs20/static/"
DATA_DIC = ROOT_DIC + "CBSData/template/"
    ...
    try:
        if request.args and len(request.args) > 0:
            ...
            if len(interactive) > 0:
                ...
                recursive_overwrite(ROOT_DIC + "CBSData/template/" + interactive_code + "/",  HOME_DIC + "temp/" + interactive_code + "/")
...
def upload_image():
    try:
        ...
        osFileServer = OSFS(ROOT_DIC + "web2py/applications/cbs20/static/temp/" + path + "/img/")
...
def manager():
    try:
        ...
        links = [{'header': 'Delete', 'body': lambda row: A('Delete', _href='javascript:detele_interactive(' + str(row.id) + ')')},
