...
def clear_solr():
    try:
        ...
        url_detele = "http://localhost:8189/solr/db/update?stream.body=<delete><query>*:*</query></delete>&commit=true"
...
def import_solr():
    clear_solr()
    headers = {'Content-Type': 'application/json'}
    url_update = "http://localhost:8189/solr/db/update/json/docs"
    url_commit = "http://localhost:8189/solr/db/update?stream.body=<commit/>"
    try:
        print("import")
...
def request_post_service(service_url, vars=None):
    headers = {'Content-Type': 'application/json'}
    req = requests.post(service_url, vars, headers)
    return req


def test_reg():
     data = {}
     data['username'] = "trunglbict@gmail.com"
     data['secret'] = "trungdepzai"
     data['fund'] = "20000"
     result = request_post_service("http://elearning.classbook.vn/service/charge.php", data)
     return result
