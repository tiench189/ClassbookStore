...
db.define_table('clsb_class',
                ...
                Field('class_order', type='string', notnull=True, default=9999,
                      label=T('Class order')),
                Field('class_sequent', type='string', notnull=True, default=1,
...
db.define_table('clsb_product',
                ...
                Field('product_status', type='string', notnull=True, default='Pending',
                      requires=IS_IN_SET(['Import', 'Pending', 'Approved', 'Denied', 'Disable'], zero='Choose status:'),
                ...
                Field('show_on', type='string', default='ANDROID_APP/IOS_APP/CBM/STORE_WEB/STORE_APP',
                      label=T('Show On')),
                Field('has_media', type='integer', default=0,
                      label=T('Has Media')),
...
db.clsb_product.product_creator.widget = SQLFORM.widgets.autocomplete(request, db.clsb_dic_creator.creator_name,
                                                                      id_field=db.clsb_dic_creator.id)
########################################
...
# db.clsb_product_metadata.product_id.widget = SQLFORM.widgets.autocomplete(request, db.clsb_product.product_title,
#                                                                           id_field=db.clsb_product.id)
# db.clsb_product_metadata.metadata_id.widget = SQLFORM.widgets.autocomplete(request, db.clsb_dic_metadata.metadata_name,
#                                                                            id_field=db.clsb_dic_metadata.id)
########################################
...
db.define_table('clsb_user',
                ...
                Field('email', type='string', length=100, notnull=True, unique=True, requires=IS_EMAIL(),
                      label=T('Email')),
                Field('phoneNumber', type='string', length=50, notnull=True, default="0987654321",
                ...
                Field('created_on', type='datetime', label=T('Created On')),
                auth.signature,
                format='%(username)s')
...
db.define_table('clsb_device',
                ...
                #      label=T('User')),
                Field('user_id', type='reference clsb_user',
                      requires=IS_EMPTY_OR(IS_IN_DB(db, 'clsb_user.id', '%(username)s')), label=T('User')),
                Field('device_serial', type='string', notnull=True, #unique=True, vuongtm -> cho phep nhieu user dang ky 1 device
...
db.define_table('clsb_download_archieve',
                ...
                Field('price', type='integer', notnull=True,
                      label=T('Price')),
                Field('pay_provider', type='integer', notnull=True,
                      label=T('Pay Provider')),
                Field('pay_cp', type='integer', notnull=True,
                      label=T('Pay CP')),
...
########################################
db.define_table('clsb_download_archieve_2014',
                Field('user_id', type='reference clsb_user', notnull=True,
                      label=T('User id')),
                Field('product_id', type='reference clsb_product', notnull=True,
                      label=T('Product')),
                ...
                auth.signature,
                format='%(product_id.product_code)s')
db.clsb_download_archieve._singular = 'Download Archieve 2104'
db.clsb_download_archieve._plural = 'Download Archieves 2104'
########################################
...
db.define_table('clsb_home_topic',
                ...
                Field('category_id', type='reference clsb_category', notnull=True,
                      label=T('Category Id')),
                Field('used_for', type='string', default='All',
                      requires=IS_IN_SET(['All', 'Classbook_manager', 'ClassbookVN']),
                      label=T('Used for')),
                Field('status', type='string', default='unpublish',
                      requires=IS_IN_SET(['default', 'published', 'unpublish']),
                      label=T('Status')),
                auth.signature,
                format='%(topic_name)s')
...
db.define_table('clsb_transaction',
                ...
                Field('face_value', type='integer', default=0, notnull=True, label=T('Face Value')),
                Field('real_value', type='integer', default=0, notnull=True, label=T('Real Value')),
                ...
                Field('site', type='string', label=T('Site')),
                Field('discount_code', type='string', label=T('Discount Code')),
                auth.signature,
                format='%(id)s')
...
db.define_table('clsb_show_hide',
                Field('value_show', type='string', notnull=True, label='Value'),
                Field('product_type', type='integer', notnull=True, label='Product Type'),
                Field('auth_id', type='integer', notnull=True, label='Auth'),
                Field('category', type='integer', notnull=True, label='Category'),
...
def check_is_root():
    if db(db[auth.settings.table_user_name].id == auth.user.id)(
                    db[auth.settings.table_user_name].is_root == True).count() == 0:
...
def check_permission(name="List", record_id=0):
    if auth.has_permission(name, "clsb_category", record_id):
        return True
    else:
        parent_id = db(db.clsb_category.id == record_id).select(
            db.clsb_category.category_parent).first().category_parent
        try:
            return check_permission(name, parent_id)
        except:
            return False
...
def get_list_category():
    ...
    if groups is None or len(groups) == 0:
        query = query(db.auth_permission.group_id == 0)
    else:
        for group in groups:
            query = query or (db.auth_permission.group_id == group.group_id)
    categories = db(db.clsb_category.id >= 0)\
        (db.clsb_category.category_parent == None).select(db.clsb_category.id, db.clsb_category.category_name, distinct=True).as_list()
...
def get_category_children(catid=0):
    if catid == 0:
        return list()
    return db(db.clsb_category.category_parent == catid).select(db.clsb_category.id, db.clsb_category.category_name)


def smartgrid(table, constraints=None, linked_tables=None,
              links=None, links_in_grid=True,
              args=None, user_signature=True,
              divider='>', breadcrumbs_class='', **kwargs):
    if constraints is None:
        constraints = dict()


    #    clsb_product.product_category/4
    args_len = len(request.args)

    tablename = "clsb_product"
    cat_id = -1
    if args_len > 2 and request.args[args_len - 2] == "clsb_product.product_category":
        cat_id = request.args[args_len - 1]
    elif tablename == str(table):
        cat_id = session.catid
        #    elif (args_len > 2 and request.args[args_len - 2].find("clsb_product.") >= 0):
    #        categories = db(db.clsb_category.id > 0).select(db.clsb_category.id)
    #        for category in categories:
    #            catids.append(category.id)

    if cat_id != -1:
        #if check_permission('List', cat_id):
        if cat_id == 0:
            dbquery = (db[tablename].product_category > 0)
        else:
            dbquery = (db[tablename].product_category == cat_id)
            categories = get_category_children(cat_id)
            for category in categories:
                dbquery |= db[tablename].product_category == category.id
        if tablename in constraints:
            constraints[tablename] = constraints[tablename] & dbquery
        else:
            constraints[tablename] = dbquery
        #else:
        #    if tablename in constraints:
        #        constraints[tablename] &= db[tablename].id == 0
        #    else:
        #        constraints[tablename] = (db[tablename].id == 0)
        #db.clsb_product.product_status.writable = check_permission('Approve', cat_id)
        db.clsb_product.product_status.writable = True
        if not 'create' in kwargs or kwargs['create']:
            #kwargs['create'] = dict(clsb_product=check_permission('Create', cat_id))
            kwargs['create'] = dict(clsb_product=True)
        if not 'deletable' in kwargs or kwargs['deletable']:
            #kwargs['deletable'] = dict(clsb_product=check_permission('Delete', cat_id))
            kwargs['deletable'] = dict(clsb_product=True)
        if not 'details' in kwargs or kwargs['details']:
            #kwargs['details'] = dict(clsb_product=check_permission('Read', cat_id))
            kwargs['details'] = dict(clsb_product=True)
        if not 'editable' in kwargs or kwargs['editable']:
            #kwargs['editable'] = dict(clsb_product=check_permission('Update', cat_id))
            kwargs['editable'] = dict(clsb_product=True)
    kwargs['maxtextlength'] = 30
    form = SQLFORM.smartgrid(table, constraints, linked_tables,
                             links, links_in_grid, args, user_signature,
                             divider, breadcrumbs_class, **kwargs)
