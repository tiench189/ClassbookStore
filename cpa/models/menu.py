...
def render_menu():
    menu = list()
    menu.append(['Trang chủ', URL('default', 'index') == URL(), URL('default', 'index')])
    if not auth.user:
        return menu
...
response.logo = IMG(_src=URL('static', 'images/img_logo_classbook.png'), _style='float:left; margin-top: 6px; height: 30px;')
