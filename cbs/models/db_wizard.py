...
db.define_table('clsb_dic_creator',
    ...
    Field('creator_image', type='upload', uploadfs=osFileServer, requires=IS_IMAGE(), notnull=True, autodelete=True,
...
db.define_table('clsb_category',
    ...
    Field('category_order', type='integer', notnull=True, default=9999,
          label=T('Order')),
    Field('category_parent', type='reference clsb_category', label='Category Parent', requires=IS_EMPTY_OR(IS_IN_DB(db, 'clsb_category.id', '%(category_name)s'))),
    auth.signature,
...
#           requires = IS_IN_SET(['SK', 'SGK1', 'SGK2', 'SGK3', 'SGK4', 'SGK5', 'SGK6', 'SGK7', 'SGK8', 'SGK9', 'SGK10', 'SGK11', 'SGK12', 'SGV'], zero = None),
    ...
    Field('device_shelf_type', type='string', notnull=True, requires=IS_IN_SET(['BOOK', 'APP', 'QUIZ'], zero=None),
...
db.define_table('clsb_order',
    ...
    Field('address', type = 'text', notnull = True,
          label = T('Address')),
    Field('country', type = 'reference clsb_country', #notnull = True, but we have old order, that dont have those fields. Then we let its null
          label = T('Country')),
