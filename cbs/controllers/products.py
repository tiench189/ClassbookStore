...
def get():
    try:
        ...
        if request.args:
            ...
            check_child = db(db.clsb_category.category_parent == cat_id).select()
            if len(check_child) == 0:
                product_query = product_query(db.clsb_product.product_category == cat_id)
            else:
                product_query = product_query(db.clsb_product.product_category == db.clsb_category.id)\
                                              (db.clsb_category.category_parent == cat_id)
        ...
        check_cache = myredis.get_cache(PRODUCT_GET + request.args(0) + version_app + str(page) + "_" + str(items_per_page))
        ...
        data = dict(page=page, items_per_page=items_per_page, total_items=total_items, total_pages=total_pages,
                    products=products)
        myredis.write_cache(PRODUCT_GET + request.args(0) + version_app + str(page) + "_" + str(items_per_page), str(json.dumps(data)), DEFAULT_TIME)
        data['cache'] = False
        return data
...
def topdownload():
    ...
    if 'version' in request.vars:
        version_app = request.vars.version
    check_cache = myredis.get_cache(TOP_DOWN + product_type + str(n) + version_app)
    if check_cache['result'] and check_cache['data'] is not None:
        data = json.loads(check_cache['data'])
        data['cache'] = True
        return data
    try:
        ...
        data = dict(items=d)
        myredis.write_cache(TOP_DOWN + product_type + str(n) + version_app, str(json.dumps(data)), DEFAULT_TIME)
        data['cache'] = False
        return data
    except Exception as e:
        print(e)
...
def toppaynocache():
    try:
        ...
        query = query(db.clsb_product.product_status.like("Approved"))
        query = query(db.clsb_download_archieve.user_id == db.clsb_user.id)\
                (db.clsb_user.test_user != 1)
        if version_app != "":
            query = query(db.clsb_product.show_on.like('%' + version_app.upper() + '%'))
        if product_type != "":
            if str(product_type).lower() == "book":
                query = query(~db.clsb_product.product_code.like("%.%"))
            else:
                query = query(db.clsb_product.product_code.like("%.%"))
        query = query(~db.clsb_product.product_code.like("Exer%"))
        ...
        for row in select_product:
            ...
            temp['category_type'] = product_type
            temp['free'] = False
            products.append(temp)
...
def toppay():
    try:
        ...
        if 'version' in request.vars:
            version_app = request.vars.version
        check_cache = myredis.get_cache(TOP_PAY + product_type + str(n) + version_app)
        if check_cache['result'] and check_cache['data'] is not None:
            data = json.loads(check_cache['data'])
            data['cache'] = True
            return data
        ...
        for row in select_product:
            ...
            products.append(temp)
        data = dict(items=products)
        myredis.write_cache(TOP_PAY + product_type + str(n) + version_app, str(json.dumps(data)), DEFAULT_TIME)
        data['cache'] = False
        return data
    except Exception as e:
        print (str(e) + " on line " + str(sys.exc_traceback.tb_lineno))
...
def topnew():
    ...
    if 'version' in request.vars:
        version_app = request.vars.version
    check_cache = myredis.get_cache(TOP_NEW + product_type + str(n) + version_app)
    if check_cache['result'] and check_cache['data'] is not None:
        data = json.loads(check_cache['data'])
        data['cache'] = True
        return data
        ...
        for row in rows:
            if len(d) < n:
                ...
                if os.path.exists(settings.home_dir + path):
                    ...
                    pass
        data = dict(items=d)
        myredis.write_cache(TOP_NEW + product_type + str(n) + version_app, str(json.dumps(data)), DEFAULT_TIME)
        data['cache'] = False
        return data
    except Exception as e:
        return str(e) + " on line " + str(sys.exc_traceback.tb_lineno)
...
def search_multiple(store_search, category_id, subject_id, class_id, page, items_per_page, limitby, category_type, version=""):
    try:
        if store_search is None:
            store_search = "*"
        url_search = "http://localhost/cbs20/solr/search.json?key=" + store_search + "&row=" + str(items_per_page) + "&start=" + str(items_per_page*page)
        #print(url_search)
...
def search_multiple_bk(store_search, category_id, subject_id, class_id, page, items_per_page, limitby, category_type, version=""):
    try:
        if store_search:
            result_id = list()
...
def search():
    try:
        ...
        if request.vars:
            ...
            keyword = khongdau(keyword).replace(" ", "+")
            if len(keyword) <= 0:
            ...
            try:
                ...
                pass
            url_search = "http://app.classbook.vn/cbs20/solr/search.json?key=" + keyword + "&row=" + str(items_per_page) + "&start=" + str(items_per_page*page)
            get_data = urllib2.urlopen(url_search)
            str_json = str(get_data.read())
            get_data = json.loads(str_json)
            total_items = int(get_data['numFound'])
