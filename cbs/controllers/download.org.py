...
#            sys.path.append('C:\Python27\Lib\site-packages\pdf2dev.zip') # Add zip file to search path
        ...
        elif serial.lower().startswith('ca') or serial.lower().startswith('eh411500002'):  # may cb02 hoac cac may cai dat classbook app:
...
#     db.auth_user.created_by.readable = True
    try:
        ...
        # if (not str.startswith(product_code, 'CP')) & (not str.startswith(product_code, 'ExerCP')):
        #     path = fs.path.pathjoin(folder, 'thumb.png')
        # else:
        #     try:
        #         cpid = None
        #         if str.find(product_code, ".") >= 0:
        ...
        #         else:
        #             cpid = int(product_code[2:-17])
        #         path = fs.path.pathjoin(settings.cp_dir, "CP%s" % cpid, 'published', product_code, 'thumb.png')
        #     except TypeError:
        #         raise HTTP(500, 'Product Code Invalid!')
...
#         return ID_NOT_EXIST
    ...
    try:
        ...
        if (not new_fund) & (new_fund != 0):
            return dict(error="Error")
        # print new_fund
        # return dict(error="Error: "+new_fund)
        print old_fund, new_fund
        fund = old_fund-new_fund

        return dict(price=fund)
    except Exception as ex:
