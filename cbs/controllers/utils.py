...
def log_20(params, insert):
    try:
        ...
        product = db(db.clsb_product.product_code == params['pcode']).select()
        if len(product) > 0:
            try:
                ...
                if insert:
                    ...
                    #     product_title=product.first()['product_title'],
                    #     product_price=params['price'],
                    #     product_id=product_id,
                    #     category_id=product.first()['product_category'],
                    #     user_id=params['userID']
                    # )
                    #
                    ...
                    #update log by id
                    # product_history = db((db.clsb30_product_history.product_id == product_id) & (db.clsb30_product_history.user_id == params['userID'])).select()
                    ...
                    #         user_id=params['userID']
                    #     )
                    # media_history = db((db.clsb30_media_history.product_id == product_id) & (db.clsb30_media_history.user_id == params['userID'])).select()
                    ...
                    if len(log_data.select()) > 0:
                        db(db.clsb_download_archieve.id == params['log_id']).update(price=params['price'], status=params['status'])

                        # data = dict(record_id=params['log_id'], table_name='clsb_download_archieve', key_unique='user_id.product_id')
...
#                  mytable.product_id==prodID).select():
                ...
                if insert:
                    ...
                    discount = 98
                    select_cp = db(db.clsb20_product_cp.product_code == db.clsb_product.product_code)\
                            (db.clsb_product.id == prodID).select()
                    if len(select_cp) > 0:
                        discount = usercp.get_discount_value(select_cp.first()['clsb20_product_cp']['created_by'], db)
                    else:
                        select_cate = db(db.clsb_product.id == prodID)\
                                (db.clsb_product.product_category == db.clsb_category.id)\
                                (~db.clsb_category.category_parent.belongs([1, 26])).select()
                        if len(select_cate):
                            discount = 30
                    ...
                    db.clsb_download_archieve.insert(user_id=params['userID'],
                                            ...
                                            pay_cp=pay_cp,
                                              ...
                                              status=params['status'],)
                    db.clsb30_product_history.insert(
                        product_title=prodID[0]['product_title'],
                        product_price=params['price'],
                        product_id=prodID,
                        category_id=prodID[0]['product_category'],
                        user_id=params['userID']
                    )
                    ...
                    # data = dict(record_id=download_id, table_name='clsb_download_archieve', key_unique='user_id.product_id')
                    insert_to_log_temp(data)
            return SUCCES
        except Exception as e:
            if not prodID:
...
def pay(username, total, product_id, oldCBM):
    ...
    try:
        ...
        if not user_id:
            return CB_0010  # Tên ??ng nh?p không t?n t?i
...
def str2price(value):
    ...
    if price[0] == '.':
        price = price[1:]
    return u'Miễn phí' if price == '0' else price + 'đ'
...
def pay_to_log(user, product, classbook_device, end_buy=False):
    ...
    if not check_free_for_classbook(product['clsb_category']['id']):
        downloaded = db(db.clsb_download_archieve.status.like("Completed"))(db.clsb_download_archieve.product_id == product['clsb_product']['id'])(db.clsb_download_archieve.user_id == user['id']).select()
        if len(downloaded) > 0:
            return True
    """
...
def make_cover_media(path, product_code, name, extension):
    try:
        ...
        if not check_media_in_zip(fs.path.pathjoin(settings.home_dir, path, product_files[0]), product_code):
            return name+"."+extension
        path_out = fs.path.pathjoin(settings.home_dir, path, name+'_media.'+extension)
        if os.path.exists(path_out):
            os.remove(path_out)
            #add for change media icon
            #return name+'_media.'+extension
        import Image
        background = Image.open(settings.home_dir+'/'+path+'/'+name+'.'+extension)
        foreground = Image.open("/home/CBSData/"+name+".png")
        foreground = foreground.resize([background.size[0]*40/100, background.size[0]*40/100])
        ...
        return e.message  + str(e) + " on line: "+str(sys.exc_traceback.tb_lineno)
...
def make_thumb_media(path, product_code, name, extension):
    import fs.path
    product_files = osFileServer.listdir(path=path, wildcard=product_code + ".[Zz][Ii][Pp]", files_only=True)
    if len(product_files) == 0:
        return False
...
def convert_cover_media(path, product_code, name, extension):
    try:
        import fs.path
        product_files = osFileServer.listdir(path=path, wildcard=product_code + ".[Zz][Ii][Pp]", files_only=True)
        if len(product_files) == 0:
        ...
        if not check_media_in_zip(fs.path.pathjoin(settings.home_dir, path, product_files[0]), product_code):
            return name+"."+extension
        try:
            os.rename(settings.home_dir+'/'+path+'/'+name+'.'+extension, settings.home_dir+'/'+path+'/'+name+'_bk.'+extension)
        except Exception as e:
            pass
        os.rename(settings.home_dir+'/'+path+'/'+name+'_media.'+extension, settings.home_dir+'/'+path+'/'+name+'.'+extension)
    except Exception as err:
        print err
...
def sync_data_to_db(get_data):
    ...
    try:
        ...
        if len(unique_data) > 0:
            check_data = db_sync.executesql("SELECT * FROM " + str(table_name) + " WHERE " + query_unique)
            if len(check_data) > 0:
                check_exist = True
        if check_exist:
...
def get_data_sync(record_id, table_name, key_unique): #record_id, table_name, key_unique
    try:
        data_result = db.executesql("SELECT * FROM " + str(table_name) + " WHERE id =" + record_id, as_dict=True)
